...
df['target'] = df['target'].map({'tested_positive': 1,
                                  'tested_negative': 0}).astype(int)

print(f"Dataset shape: {df.shape}")
print(f"Diabetic cases: {df['target'].sum()} / {len(df)}")
...
plt.tight_layout()
plt.savefig('diabetes_eda.png', dpi=100)
plt.show()
print("EDA saved!")
...
plt.tight_layout()
plt.savefig('diabetes_results.png', dpi=100)
plt.show()

rf = results['Random Forest']['model']
feat_imp = pd.Series(rf.feature_importances_, index=X.columns).sort_values(ascending=False)
plt.figure(figsize=(10, 5))
feat_imp.plot(kind='bar', color='orange', edgecolor='black')
plt.title('Feature Importance — Random Forest (Diabetes)')
plt.ylabel('Importance')
plt.tight_layout()
plt.savefig('diabetes_feature_importance.png', dpi=100)
plt.show()
